...
function App() {
  ...
  const fazerLogin = async (e) => {
    e.preventDefault();
    try {
      const resp = await axios.post(`${API_URL}/login`, { login: loginUser, senha: loginSenha });
      setUsuarioLogado(resp.data);
      setTelaAtual('inicio');
      mostrarNotificacao('Bem-vindo!', 'success');
    } catch { mostrarNotificacao('Usuário ou senha incorretos!', 'error'); }
  };
  ...
  const TelaInicio = () => (
    <div className="page-container" style={{flexDirection: 'column', alignItems: 'center', minHeight: 'calc(100vh - 80px)'}}>
  ...
  const TelaSaidaInsumo = () => {
    ...
    useEffect(() => {
      axios.get(`${API_URL}/insumos`).then(res => setListaInsumos(res.data.sort((a, b) => a.nome.localeCompare(b.nome))));
    ...
    const registrarSaida = async (e) => {
      ...
      try {
        ...
        mostrarNotificacao('Saída registrada!', 'success');
      } catch { mostrarNotificacao('Erro ao registrar.', 'error'); }
    };
  ...
  if (!usuarioLogado) {
    return (
      <div className="login-screen">
        <div className="notification-container">{notificacoes.map(n => <div key={n.id} className={`notification ${n.tipo}`}>{n.mensagem}</div>)}</div>
